...
public class Comp {
	...
	public void run(String[] args) {
		...
		if ( args.length < 1 || args.length > 2 ) {
			...
			for (int i = 0; i < args.length; i++) {
				...
				file = new File(fileOrDirName);
				if (!file.exists() || !file.canRead()) {
					String msg = "Either the file " + fileOrDirName + " does not exist or it cannot be read";
			...
			/**
			 * // Just a test StatementAssert stat = new StatementAssert( new
			 * VariableExpr( new Variable("name", Type.intType)), 10, "operator
			 * '+' in '1 + 1 == 2'"); PW pw = new PW(); pw.set(outError);
			 * stat.genC(pw); outError.flush();
			 * 
			 */
	...
	private void printReport(int numSourceFiles, PrintWriter report) {
		...
		if (numSourceFiles - numSourceFilesWithErros != 0) {
			report.println(this.wereButShouldNotList.size() + " erros foram sinalizados em "
					+ (numSourceFiles - numSourceFilesWithErros) + " arquivos sem erro ("
		...
		if (correctList.size() > 0) {
			...
			report.print(
					"mensagens emitidas pelo compilador s�o compat�veis com as mensagens de erro sugeridas pelas chamadas aos ");
